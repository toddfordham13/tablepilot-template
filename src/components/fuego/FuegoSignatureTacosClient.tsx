...
import type { MenuContent, MenuItem } from "@/lib/menus/types"

type FuegoSignatureTacosClientProps = {
...
function getSignatureTacos(menuContent: MenuContent): MenuItem[] {
  const tacosSection = menuContent.sections.find(
    (section) => section.id === "tacos",
  )

  if (!tacosSection) return []

  return tacosSection.items
    .filter((item) => item.visible !== false)
    .slice()
    .sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0))
}

export default function FuegoSignatureTacosClient({
  menuContent,
}: FuegoSignatureTacosClientProps) {
  const tacos = useMemo(() => getSignatureTacos(menuContent), [menuContent])
    ...
    >
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 md:px-8 md:py-16">
        ...
        {menuContent.description ? (
          <p
            className="mt-6 max-w-3xl text-[0.98rem] leading-7 sm:text-base sm:leading-8"
        ...
        <div className="mt-8 grid gap-4 sm:mt-10 sm:gap-5 md:grid-cols-2">
          {tacos.map((item) => (
            <div
              key={item.id}
              className="group relative rounded-[20px] border bg-white p-5 transition duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(27,19,17,0.10)] sm:rounded-[24px] sm:p-7 md:p-8"
              style={{
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <h4
                  className="text-[2rem] font-black leading-[0.95] sm:text-3xl md:text-[34px]"
                  style={{
                    color: "#1B1311",
                ...
                {item.price ? (
                  <span
                    className="shrink-0 text-sm font-bold uppercase tracking-[0.12em] sm:text-base"
                    style={{ color: "#7A1E1E" }}
                  >
                    {item.price}
                  </span>
